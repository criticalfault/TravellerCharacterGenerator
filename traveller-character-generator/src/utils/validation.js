...
export const validateBackgroundSkills = (skills, eduDM) => {
  ...
  if (usedPoints > maxPoints) {
    errors.push(
      `Too many background skill points used (${usedPoints}/${maxPoints})`
    );
  }
...
export const validateCareerSelection = (character, careerName) => {
  ...
  if (
    ...
    !validateAttributes(character.attributes).isValid
  ) {
...
export const validateCompleteCharacter = character => {
  ...
  const expectedAge =
    18 +
    character.careerHistory.reduce((total, career) => total + career.terms, 0) *
      4;
  if (character.age !== expectedAge) {
    errors.push(
      `Age inconsistency: expected ${expectedAge}, got ${character.age}`
    );
  }
...
export const getCharacterProgress = character => {
  const steps = [
    ...
    },
    {
      ...
      completed: character.backgroundSkillsSelected,
    },
    {
      id: 'career',
      name: 'Career',
      completed: character.careerHistory && character.careerHistory.length > 0,
    },
    {
      id: 'mustering',
      name: 'Mustering Out',
      completed: character.benefitRolls === 0,
    }, // All benefits used
    {
      id: 'complete',
      name: 'Complete',
      completed: validateCompleteCharacter(character).isValid,
    },
  ];
