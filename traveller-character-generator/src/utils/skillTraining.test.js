...
describe('Skill Training System', () => {
  ...
  describe('getAvailableSkillTables', () => {
    test('returns basic tables for enlisted character', () => {
      const tables = getAvailableSkillTables(mockCareer, 'Law Enforcement', mockCharacter);
    ...
    test('includes officer table for commissioned characters', () => {
      const tables = getAvailableSkillTables(mockCareer, 'Law Enforcement', mockCommissionedCharacter);
    ...
    test('excludes advanced education for low EDU characters', () => {
      ...
      const tables = getAvailableSkillTables(mockCareer, 'Law Enforcement', lowEduCharacter);
      
      expect(tables.map(t => t.name)).not.toContain('Advanced Education');
    });

    test('handles missing career data gracefully', () => {
      const tables = getAvailableSkillTables(null, 'Law Enforcement', mockCharacter);
  ...
  describe('rollOnSkillTable', () => {
    beforeEach(() => {
      roll2d6.mockReturnValue({ total: 4, dice: [2, 2], formatted: '4 (2, 2)' });
    });
    ...
    test('handles missing skill entries gracefully', () => {
      roll2d6.mockReturnValue({ total: 7, dice: [3, 4], formatted: '7 (3, 4)' });
  ...
  describe('applySkillTraining', () => {
    ...
    test('applies skill increases correctly', () => {
      ...
      applySkillTraining(mockCharacter, skillResult, mockDispatch, CHARACTER_ACTIONS, mockAddSkill, mockUpdateAttribute);
  ...
  describe('canAccessAdvancedEducation', () => {
    ...
    test('handles missing requirements gracefully', () => {
      const careerWithoutAdvanced = {
        skills_and_training: {
          personal_development: mockCareer.skills_and_training.personal_development
        }
      };
      
      const canAccess = canAccessAdvancedEducation(mockCharacter, careerWithoutAdvanced);
  ...
  describe('validateSkillTrainingPrerequisites', () => {
    test('validates successful prerequisites', () => {
      const validation = validateSkillTrainingPrerequisites(mockCharacter, mockCareer, 'Law Enforcement');
    ...
    test('identifies missing career', () => {
      ...
      const validation = validateSkillTrainingPrerequisites(characterWithoutCareer, mockCareer, 'Law Enforcement');
    ...
    test('identifies missing career data', () => {
      const validation = validateSkillTrainingPrerequisites(mockCharacter, null, 'Law Enforcement');
    ...
    test('identifies missing assignment', () => {
      const validation = validateSkillTrainingPrerequisites(mockCharacter, mockCareer, null);
    ...
    test('identifies missing skill training data', () => {
      ...
      const validation = validateSkillTrainingPrerequisites(mockCharacter, careerWithoutSkills, 'Law Enforcement');
      
      expect(validation.valid).toBe(false);
      expect(validation.issues).toContain('No skill training data available for this career');
    });
  });
});