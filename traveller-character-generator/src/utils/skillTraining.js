...
export const getAvailableSkillTables = (career, assignment, character) => {
  ...
  if (
    skillsData.advanced_education &&
    skillsData.advanced_education_requirements
  ) {
...
export const rollOnSkillTable = skillTable => {
  ...
  if (!skillEntry) {
    console.warn(
      `No skill entry found for roll ${roll} on table ${skillTable.name}`
    );
    return null;
  }
...
export const parseSkillEntry = skillEntry => {
  if (Array.isArray(skillEntry)) {
    ...
    return {
      type: 'choice',
      options: skillEntry
        .map(skill => parseSkillEntry(skill))
        .map(parsed => parsed.skills[0]),
    };
  }
 ...
 */
export const applySkillTraining = (
  character,
  skillResult,
  dispatch,
  CHARACTER_ACTIONS,
  ...
  if (!skillResult) {
    console.error('applySkillTraining: skillResult is null or undefined');
    return;
  }

  if (!skillResult.skills) {
    console.error(
      'applySkillTraining: skillResult.skills is null or undefined'
    );
    return;
  }

  if (!skillResult.skills.skills || !Array.isArray(skillResult.skills.skills)) {
    console.error(
      'applySkillTraining: skillResult.skills.skills is not an array',
      skillResult.skills
    );
    return;
  }
  ...
  if (
    ...
    console.error('applySkillTraining: Missing required parameters');
    return;
  }

  const skillsToApply = skillResult.skills.skills;

  try {
    skillsToApply.forEach(skill => {
      if (!skill || typeof skill !== 'object') {
        console.warn('applySkillTraining: Invalid skill object', skill);
        return;
      }

      if (skill.isAttribute) {
        // Apply attribute increase
        if (!skill.name || !character.attributes.hasOwnProperty(skill.name)) {
          console.warn(
            'applySkillTraining: Invalid attribute name',
            skill.name
          );
          return;
        }
        const currentValue = character.attributes[skill.name] || 0;
        const newValue = currentValue + (skill.level || 1);
        updateAttribute(skill.name, newValue);
      } else {
        // Apply skill increase with proper stacking
        if (!skill.name) {
          console.warn('applySkillTraining: Invalid skill name', skill.name);
          return;
        }
        addSkill(skill.name, skill.level || 1);
      }
    });
    ...
    dispatch({
      ...
      payload: {
        type: 'skill_training',
        table: skillResult.table || 'Unknown Table',
        roll: skillResult.roll || 0,
        description: `Trained on ${skillResult.table || 'Unknown Table'}: ${skillsToApply.map(s => s.displayName || s.name || 'Unknown Skill').join(', ')}`,
      },
    });
  } catch (error) {
    console.error('applySkillTraining: Error applying skill training', error);
  }
};
 ...
 */
export const handleSkillChoice = (
  character,
  choiceOptions,
  selectedOption,
  dispatch,
...
export const canAccessAdvancedEducation = (character, career) => {
  if (!career?.skills_and_training?.advanced_education_requirements)
    return false;

  const eduReq =
    career.skills_and_training.advanced_education_requirements.EDU || 8;
 ...
 */
export const validateSkillTrainingPrerequisites = (
  character,
  career,
  assignment
) => {
