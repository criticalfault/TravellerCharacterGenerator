...
import { generateAttributes2d6, generateAttributes3d6DropLowest, roll2d6, roll3d6DropLowest } from '../../utils/dice';

export default function AttributesTab() {
  const { character, updateAttribute, getAttributeModifier, dispatch, CHARACTER_ACTIONS } = useCharacter();
  ...
  return (
    <div className="attributes-tab">
      ...
      <p>Roll or set your character's attributes. In Traveller, attributes range from 2-12 (2d6) or 2-18+ and determine your character's basic capabilities.</p>
      ...
      <div className="attributes-section">
        <div className="attribute-summary">
          <div className="summary-stats">
            ...
            <span className="stat-item">
              <strong>Average:</strong> {(calculateAttributeTotal() / 6).toFixed(1)}
            </span>
            <span className="stat-item">
              <strong>Physical Total:</strong> {character.attributes.STR + character.attributes.DEX + character.attributes.END}
            </span>
          </div>
        </div>
        ...
        <div className="attribute-actions">
          ...
          >
            {character.attributesLocked ? 'Unlock Attributes' : 'Lock Attributes'}
          </button>
          <button 
            className="btn btn-secondary"
                ...
                {roll.singleRoll && (
                  <div className="history-single">
                    <span className="single-result">
                      ...
                      {roll.singleRoll.dice && ` (${roll.singleRoll.dice.join(', ')})`}
                      {roll.singleRoll.keptDice && ` (kept: ${roll.singleRoll.keptDice.join(', ')}, dropped: ${roll.singleRoll.droppedDie})`}
                    </span>
                  </div>
                )}
              </div>
      ...
      <div className="attribute-info">
        ...
        <div className="info-grid">
          <div className="info-item">
            ...
            <ul>
              <li><strong>2d6:</strong> Standard Traveller method (2-12 range)</li>
              <li><strong>3d6 drop lowest:</strong> Higher average results (2-12 range, better odds)</li>
            </ul>
          </div>
          <div className="info-item">
          ...
          <div className="info-item">
            ...
            <p>Your physical damage capacity is STR + DEX + END = {character.attributes.STR + character.attributes.DEX + character.attributes.END}</p>
          </div>
        </div>
          ...
          <div className="details-grid">
            {Object.entries(character.attributes).map(([attr, value]) => (
              <div key={attr} className="detail-item">
                <h5>{attr} ({value}) - DM {getAttributeModifier(value) >= 0 ? '+' : ''}{getAttributeModifier(value)}</h5>
