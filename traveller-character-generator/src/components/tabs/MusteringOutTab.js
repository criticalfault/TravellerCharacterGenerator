...
export default function MusteringOutTab() {
  ...
  const benefitRolls = character.benefitRollsState;
  const cashBenefitsUsed = character.cashBenefitsUsed;
  const selectedBenefits = character.selectedBenefits;
  ...
  // Initialize benefit rolls when component mounts or career history changes
  useEffect(() => {
    const totalRolls = calculateBenefitRolls();
    
    // Only initialize if we don't have benefit rolls or if the count has changed
    if (!benefitRolls || benefitRolls.length !== totalRolls) {
      ...
      dispatch({
        type: CHARACTER_ACTIONS.SET_BENEFIT_ROLLS_STATE,
        payload: rolls
      });
    }
  }, [character.careerHistory, benefitRolls, dispatch, CHARACTER_ACTIONS]);
  ...
  const handleBenefitRoll = (rollId, benefitType) => {
    ...
    // Update the roll in persistent state
    dispatch({
      type: CHARACTER_ACTIONS.UPDATE_BENEFIT_ROLL,
      payload: {
        id: rollId,
        updates: { used: true, result: benefit, type: benefitType, diceRoll: rollResult }
      }
    });
    ...
    if (benefitType === 'cash') {
      dispatch({
        type: CHARACTER_ACTIONS.SET_CASH_BENEFITS_USED,
        payload: cashBenefitsUsed + 1
      });
    }
  ...
  const processBenefit = (benefit) => {
    ...
    dispatch({
      type: CHARACTER_ACTIONS.ADD_SELECTED_BENEFIT,
      payload: benefit
    });
  };
  ...
  const totalRolls = calculateBenefitRolls();
  const usedRolls = benefitRolls ? benefitRolls.filter(r => r.used).length : 0;
  ...
  return (
    <div className="mustering-out-tab">
      <h2>Mustering Out</h2>
      <p>
        Collect your final benefits from your career(s) before beginning your
        adventuring life.
      </p>
      ...
      <div className="benefit-rolls">
        ...
        <div className="rolls-grid">
          {benefitRolls && benefitRolls.map(roll => (
      ...
      <div className="current-benefits">
        <h3>Current Character Status</h3>
      ...
      <style jsx>{`
        ...
        .equipment-list, .cyberware-list {
          list-style-type: none;
          padding: 0;
        }
        
        .equipment-list li, .cyberware-list li {
          background: white;
          margin: 5px 0;
          padding: 8px;
          border-radius: 4px;
