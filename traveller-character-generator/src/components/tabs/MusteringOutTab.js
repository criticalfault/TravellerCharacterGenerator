...
export default function MusteringOutTab() {
  ...
  const availableCareers = character.careerHistory.filter(
    career => career.terms > 0
  );
  ...
  return (
    <div className="mustering-out-tab">
              ...
              {availableCareers.map((career, index) => {
                ...
                return (
                  <div key={index} className="career-option">
                    <h5>
                      {career.career} ({career.assignment})
                    </h5>
                    <p>
                      Terms: {career.terms}, Rank: {career.rank} (
                      {career.rankTitle})
                    </p>
                    <p>
                      Benefit DM: {benefitDM >= 0 ? '+' : ''}
                      {benefitDM}
                    </p>
          ...
          <div className="roll-entries">
            {rollHistory.map((entry, index) => (
              <div key={index} className="roll-entry">
                <strong>{entry.type} Roll</strong> ({entry.career}): Rolled{' '}
                {entry.roll} (table: {entry.clampedRoll})
                {entry.dm !== 0 &&
                  ` with DM ${entry.dm >= 0 ? '+' : ''}${entry.dm}`}
                →{' '}
                <em>
                  {Array.isArray(entry.benefit)
                    ? entry.benefit.join(' or ')
                    : entry.benefit}
                </em>
              </div>
            ))}
      ...
      <div className="mustering-out-info">
        ...
        <p>
          When you leave a career (voluntarily or involuntarily), you receive
          benefit rolls based on your terms of service and rank achieved. These
          benefits represent your savings, equipment, and connections gained
          during your career.
        </p>
        <p>
          <strong>Cash Benefits:</strong> Guaranteed credits based on your
          career and rank.
        </p>
        <p>
          <strong>Material Benefits:</strong> Equipment, weapons, attribute
          increases, and other valuable items. These can be more valuable than
          cash but are random.
        </p>
        <p>
          <strong>Benefit DM:</strong> Higher ranks provide dice modifiers to
          benefit rolls, increasing your chances of better results.
        </p>
      </div>
    </div>
  );
