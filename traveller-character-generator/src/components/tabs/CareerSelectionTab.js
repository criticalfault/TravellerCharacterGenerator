...
export default function CareerSelectionTab() {
  ...
  const handleSuccessfulQualification = (career, careerName, assignment) => {
    ...
    if (career.skills_and_training?.service_skills) {
      Object.values(career.skills_and_training.service_skills).forEach(
        skill => {
  ...
  return (
    <div className="career-selection-tab">
      ...
      <p>
        Choose your first career path. Each career has different requirements
        and opportunities.
      </p>
            ...
            return (
              ...
              >
                <h4 className="career-name">
                  {careerName.charAt(0).toUpperCase() + careerName.slice(1)}
                </h4>
                {qualInfo && (
                  <div className="qualification-info">
                    <p>
                      Qualification: {qualInfo.attribute} {qualInfo.baseTarget}+
                    </p>
                    <p>
                      Your modifier: {qualInfo.modifier >= 0 ? '+' : ''}
                      {qualInfo.modifier}
                    </p>
              ...
              return qualInfo ? (
                <div>
                  <p>
                    Career:{' '}
                    <strong>
                      {selectedCareer.charAt(0).toUpperCase() +
                        selectedCareer.slice(1)}
                    </strong>
                  </p>
                  <p>
                    Assignment: <strong>{selectedAssignment}</strong>
                  </p>
                  <p>
                    Qualification Roll:{' '}
                    <strong>
                      {qualInfo.attribute} {qualInfo.baseTarget}+
                    </strong>
                  </p>
                  <p>
                    Your {qualInfo.attribute}:{' '}
                    <strong>
                      {character.attributes[qualInfo.attribute]} (DM:{' '}
                      {qualInfo.modifier >= 0 ? '+' : ''}
                      {qualInfo.modifier})
                    </strong>
                  </p>
                  <p>
                    Target Number:{' '}
                    <strong>{qualInfo.totalTarget}+ on 2d6</strong>
                  </p>
                </div>
              ) : null;
            })()}
      ...
      {qualificationResult && (
        <div className="qualification-result">
          ...
          <div
            className={`result-display ${qualificationResult.success ? 'success' : 'failure'}`}
          >
            <div className="roll-details">
              <p>
                <strong>Career:</strong>{' '}
                {qualificationResult.career.charAt(0).toUpperCase() +
                  qualificationResult.career.slice(1)}
              </p>
              <p>
                <strong>Assignment:</strong> {qualificationResult.assignment}
              </p>
              <p>
                <strong>Roll:</strong> {qualificationResult.roll.formatted}
              </p>
              <p>
                <strong>Target:</strong> {qualificationResult.target}+
              </p>
              <p>
            ...
            {qualificationResult.success ? (
              <div className="success-message">
                ...
                <p>
                  You have been accepted into the {qualificationResult.career}{' '}
                  career as a {qualificationResult.assignment}.
                </p>
                <p>
                  You receive basic training in all service skills at level 0.
                </p>
              </div>
            ) : (
              <div className="failure-message">
                ...
                <p>
                  You did not meet the requirements for the{' '}
                  {qualificationResult.career} career.
                </p>
                <p>
                  You may choose to become a Drifter or try qualifying for a
                  different career.
                </p>
              </div>
            )}
      ...
      {commissionAttempt &&
        commissionAttempt.available &&
        !commissionAttempt.attempted && (
          <div className="commission-section">
            <h3>Commission Opportunity</h3>
            <p>
              As a military career, you have the opportunity to attempt to
              become an officer.
            </p>
            <div className="commission-details">
              {(() => {
                const [attr, target] = Object.entries(
                  commissionAttempt.requirement
                )[0];
                const modifier = getAttributeModifier(
                  character.attributes[attr]
                );
                return (
                  <div>
                    <p>
                      <strong>Commission Roll:</strong> {attr} {target}+
                    </p>
                    <p>
                      <strong>Your {attr}:</strong> {character.attributes[attr]}{' '}
                      (DM: {modifier >= 0 ? '+' : ''}
                      {modifier})
                    </p>
                    <p>
                      <strong>Target:</strong> {target + modifier}+ on 2d6
                    </p>
                  </div>
                );
              })()}
      ...
      {commissionAttempt && commissionAttempt.attempted && (
        <div className="commission-result">
          ...
          {commissionAttempt.declined ? (
            ...
            <div
              className={`result-display ${commissionAttempt.success ? 'success' : 'failure'}`}
            >
              <p>
                <strong>Roll:</strong> {commissionAttempt.roll.formatted}
              </p>
              <p>
                <strong>Result:</strong>{' '}
                <span
                  className={
                    commissionAttempt.success ? 'success-text' : 'failure-text'
                  }
                >
              ...
              {commissionAttempt.success ? (
                <p>
                  Congratulations! You have been commissioned as an officer.
                </p>
              ) : (
                <p>
                  Commission attempt failed. You remain in the enlisted ranks.
                </p>
              )}
            </div>
          )}
      ...
      {careerStarted && (
        <div className="career-started">
          ...
          <div className="career-summary">
            <p>
              <strong>Career:</strong>{' '}
              {selectedCareer.charAt(0).toUpperCase() + selectedCareer.slice(1)}
            </p>
            <p>
              <strong>Assignment:</strong> {selectedAssignment}
            </p>
            <p>
              <strong>Status:</strong>{' '}
              {commissionAttempt?.success ? 'Officer' : 'Enlisted'}
            </p>
            <p>
              You have received basic training and are ready to begin your first
              term.
            </p>
          </div>
      ...
      <div className="career-info">
        ...
        <p>
          Each career offers different skills, advancement opportunities, and
          risks. Consider your character's attributes when choosing a career, as
          they affect qualification chances and career progression.
        </p>
        <p>
          If you fail qualification for your chosen career, you may be drafted
          into a random career or choose to become a Drifter.
        </p>
      </div>
    </div>
  );
