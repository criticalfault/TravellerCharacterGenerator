...
describe('RaceSelectionTab', () => {
  test('renders race selection interface', () => {
    ...
    expect(screen.getByText('Choose your character\'s species. Each species has unique traits and attribute modifiers.')).toBeInTheDocument();
  });
  ...
  test('displays all available races', () => {
    ...
    Object.keys(raceData).forEach(raceName => {
      expect(screen.getByRole('button', { name: raceName })).toBeInTheDocument();
    });
  });
