...
describe('AttributesTab', () => {
  ...
  test('rolls all attributes with 3d6 drop lowest method', () => {
    ...
    const rollButton = screen.getByText(
  ...
  test('shows detailed attribute descriptions', () => {
    ...
    const physicalDescriptions = screen.getAllByText(
      /Physical power and muscle/
    );
    expect(physicalDescriptions.length).toBeGreaterThan(0);

    const agilityDescriptions = screen.getAllByText(
      /Agility, reflexes, and fine motor control/
    );
    expect(agilityDescriptions.length).toBeGreaterThan(0);

    const staminaDescriptions = screen.getAllByText(
      /Stamina, health, and constitution/
    );
