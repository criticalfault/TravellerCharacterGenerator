...
const TestComponent = () => {
  const { character, dispatch, updateAttribute, getAttributeModifier } =
    useCharacter();
  ...
  return (
    <div>
      ...
      <div data-testid="str-modifier">
        {getAttributeModifier(character.attributes.STR)}
      </div>
...
describe('CharacterContext', () => {
  ...
  test('prevents duplicate career events', () => {
    const { result } = renderHook(() => useCharacter(), {
      wrapper: CharacterProvider,
    });

    // Start a career first
    act(() => {
      result.current.startCareer('agent', 'Law Enforcement');
    });
    ...
    const eventData = {
      ...
      roll: 8,
      target: 6,
      description: 'Survival roll: 8 vs 6 = SUCCESS',
    };

    act(() => {
      result.current.dispatch({
        type: result.current.CHARACTER_ACTIONS.ADD_CAREER_EVENT,
        payload: eventData,
      });
    });
    ...
    expect(result.current.character.careerHistory[0].events).toHaveLength(1);
    expect(result.current.character.careerHistory[0].events[0].type).toBe(
      'survival'
    );
  });
});
