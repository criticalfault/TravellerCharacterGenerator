  ...
  switch (action.type) {
    ...
    case CHARACTER_ACTIONS.UPDATE_ATTRIBUTE:
      ...
      return { 
        ...
        damage: { 
          ...state.damage, 
          max: calculateMaxDamage(newAttributes.STR, newAttributes.DEX, newAttributes.END) 
        }
      };
    ...
    case CHARACTER_ACTIONS.ADD_SKILL:
      return {
        ...
        skills: {
          ...state.skills,
          [action.payload.skill]: (state.skills[action.payload.skill] || 0) + (action.payload.level || 1)
        }
      };
    ...
    case CHARACTER_ACTIONS.ADD_CAREER_EVENT:
      ...
      if (historyWithEvent.length > 0) {
        historyWithEvent[historyWithEvent.length - 1].events.push({
    ...
    case CHARACTER_ACTIONS.SET_ADVANCEMENT_DM:
      return {
        ...state,
        tempModifiers: { ...state.tempModifiers, advancementDM: action.payload }
      };
...
export const CharacterProvider = ({ children }) => {
  ...
  const startCareer = (career, assignment, rankTitle = "", commissioned = false) => {
    dispatch({
